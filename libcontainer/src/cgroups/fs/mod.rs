...
lazy_static! {
	...
	pub static ref DABBRS: Vec<String> = {
		let m = vec!["B".to_string(), "KB".to_string(), "MB".to_string(),
		"GB".to_string(), "TB".to_string(), "PB".to_string(),
		"EB".to_string(), "ZB".to_string(), "YB".to_string()];
		m
	};

	pub static ref BABBRS: Vec<String> = {
		let m = vec!["B".to_string(), "KiB".to_string(), "MiB".to_string(),
		"GiB".to_string(), "TiB".to_string(), "PiB".to_string(),
		"EiB".to_string(), "ZiB".to_string(), "YiB".to_string()];
		m
	};
...
impl Cpu {
	fn get_stats(&self, dir: &str) -> Result<ThrottlingData> {
		...
		Ok(ThrottlingData {
			periods: *h.get("nr_periods")
					  .unwrap(),
			throttled_periods: *h.get("nr_throttled")
								.unwrap(),
			throttled_time: *h.get("throttled_time")
							 .unwrap(),
			unknown_fields: UnknownFields::default(),
			cached_size: CachedSize::default(),
		})
...
impl HugeTLB {
	fn get_stats(&self, dir: &str) -> Result<HashMap<String, HugetlbStats>> {
		...
		for pagesize in HUGEPAGESIZES.iter() {
			let fusage = format!("{}.{}.{}", HUGETLB_BASE, pagesize,
								HUGETLB_USAGE);
			let fmax = format!("{}.{}.{}", HUGETLB_BASE, pagesize,
								HUGETLB_MAX_USAGE);
			let ffailcnt = format!("{}.{}.{}", HUGETLB_BASE, pagesize,
								HUGETLB_FAILCNT);
